...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            mostLikelyPosn = dist.argMax()
            distance = self.distancer.getDistance(mostLikelyPosn, pacmanPosition)
            if distance < closestDist:
                closestDist = distance
                GhostLoc = mostLikelyPosn

        minDist = float('inf')
        bestAction = None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            mazeDist = self.distancer.getDistance(successorPosition, GhostLoc)
            if mazeDist < minDist:
                bestAction = action
                minDist = mazeDist
